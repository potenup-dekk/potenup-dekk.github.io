...
문제는 쉐어덱에 참여한 멤버들이 동시에 카드를 보고 `LIKE` 스와이프를 날리는 상황에서 발생합니다.
만약 현재 덱에 49장의 카드가 있을 때, 3명의 사용자가 0.001초 차이로 `LIKE`를 누른다면 어떻게 될까요? 3개의 스레드가 동시에 `deckCardRepository.countByDeckId`를 호출하여 현재 카드 수를 49장으로 읽게 되고, 모두 제한 검증을 통과해 결과적으로 덱에 **52장**의 카드가 뚫고 들어오는 **초과 저장(Lost Update) 버그**가 발생합니다.
